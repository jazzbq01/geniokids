...
export default function ColorSortGame({ activity, question, onComplete, onFail }: Props) {
  ...
  const instruction =
    ...
    question?.prompt ??
    'Arrastra cada cubo al color correcto.';

  const [selectedItem, setSelectedItem] = useState<any | null>(null);
  const [placed, setPlaced] = useState<Record<string, string>>({});
  const [message, setMessage] = useState('Toca un cubo y luego toca su color.');
  ...
  function handleSelectItem(item: any) {
    ...
    setMessage(`Ahora toca el color: ${item.label}`);
    playAudio(audio.select ?? audio.listen);
  }

  function handleTargetClick(target: any) {
    if (!selectedItem) {
      setMessage('Primero toca un cubo.');
      playAudio(audio.select ?? audio.listen);
      return;
    }

    if (target.accepts !== selectedItem.type) {
      setMessage('Ese color no corresponde. Intenta otra vez.');
